---
layout:     post
title:      JS逆向：某某文书网反爬虫JS加密策略与破解方法
subtitle:   
date:       2022-01-27
author:     dex0423
...
- 打开网站，在输入框输入查询关键字【吴*凡】，分析 response 发现找不到返回的数据，但是却能找到以下内容：
  ...
  ![]({{site.baseurl}}/img-post/文书网-1.png)
...
- 使用 js 调试工具，对 response 数据进行解密，得到下面的结果，破解成功：
  ...
  ![]({{site.baseurl}}/img-post/文书网-5.png)
...
- 分析请的参数，发现 Form Data 里的参数
  ...
  ![]({{site.baseurl}}/img-post/文书网-6.png)

##### 2.2. pageId

```pageId: 953feda22cf2bf18b9548aaa05152120```
- 通过分析回溯，找到 pageId 的生成函数为 ```uuid()```；
  ![]({{site.baseurl}}/img-post/文书网-7.png)

- 继续查找得到 uuid 函数的生成逻辑如下：
  ![]({{site.baseurl}}/img-post/文书网-8.png)
...
- 通过分析我们发现这里的数值都是二进制的，查找关键字 ```strTobinary``` 或者查找  ```ciphertext``` 然后断点调试得到参数生成逻辑，如下图：
  ![]({{site.baseurl}}/img-post/文书网-9.png)  
...
- 分析 [{"key":"s21","value":"吴*凡"}]
  ```key``` 的值为 ```s21``` 表示案件类型为 ```全文```，
  如下图：
  ![]({{site.baseurl}}/img-post/文书网-10.png)  
  ```value``` 表示查找对象
...
##### 2.9. __RequestVerificationToken

- 通过分析，得到生成逻辑如下：
  ![]({{site.baseurl}}/img-post/文书网-11.png)  

  ![]({{site.baseurl}}/img-post/文书网-12.png)

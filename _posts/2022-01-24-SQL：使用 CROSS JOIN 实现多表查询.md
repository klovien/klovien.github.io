...
```aidl
SELECT
    c.calendar_date, e.employee_name,a.clock_in,a.clock_out,
    CASE 
...
ON
    (
		a.check_date=c.calendar_date 
        AND 
        a.emp_id=e.employee_id)
WHERE
	c.calendar_year=2021 
    AND 
    c.calendar_month=1 AND c.is_work_day='Y'
AND
	(
		a.id IS NULL
        OR
        a.clock_out IS NULL
        OR
        EXTRACT(HOUR FROM a.clock_in)>9
        OR
